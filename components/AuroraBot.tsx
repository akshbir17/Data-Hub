...
export const AuroraBot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Welcome! I'm the DS Hub Assistant. Ask me anything about your 3rd Sem subjects." }
  ]);
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    const responseText = await getAuroraResponse(text, messages);
  ...
  return (
    <div className="fixed bottom-8 right-8 z-50">
      {isOpen ? (
        <div className="w-[350px] h-[500px] bg-white rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.1)] flex flex-col overflow-hidden border border-slate-100 animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-slate-900 p-6 text-white flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center font-black text-[10px]">DS</div>
              <span className="font-black text-xs uppercase tracking-widest">Hub Assistant</span>
            </div>
          ...
          <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-5 bg-white">
          ...
          <div className="p-4 border-t border-slate-50">
            <div className="flex gap-2 bg-slate-50 p-2 rounded-2xl">
              <input
                ...
                placeholder="Ask about Math, OS, R..."
                className="flex-1 bg-transparent px-3 py-2 text-xs outline-none font-medium"
              />
              <button 
                onClick={() => handleSend()}
                disabled={isLoading || !input.trim()}
                className="p-2 bg-slate-900 text-white rounded-xl disabled:opacity-30 transition-opacity"
              >
        ...
        >
          <svg className="group-hover:rotate-12 transition-transform" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>
        </button>
      )}
    </div>
  );
